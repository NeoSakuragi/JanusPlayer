...
rsync -avq -e "ssh -i ~/.ssh/id_ed25519" \
    /home/bruno/CLProjects/Janus/server-go/{main.go,auth.go,pipeline.go,go.mod,go.sum} \
    $VPS:/root/janus-server/

echo "2. Building on VPS..."
$SSH $VPS "cd /root/janus-server && CGO_ENABLED=1 go build -o janus-server main.go auth.go pipeline.go 2>&1"
